...
public class AnomalyController {
    ...
    @GetMapping("/{id}")
    public ResponseEntity<Anomaly> getAnomalyById(@PathVariable @NonNull Long id) {
    ...
    public ResponseEntity<Anomaly> createAnomaly(@RequestBody Anomaly anomaly,
            @RequestParam @NonNull Long electionId) {
    ...
    public ResponseEntity<Anomaly> updateStatus(@PathVariable @NonNull Long id, @RequestBody Map<String, String> body) {
        return ResponseEntity.ok(anomalyService.updateAnomalyStatus(id, body.get("status")));
    }

    @DeleteMapping("/{id}")
    @PreAuthorize("hasRole('ADMIN')")
    public ResponseEntity<?> deleteAnomaly(@PathVariable @NonNull Long id) {
